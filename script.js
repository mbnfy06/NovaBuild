...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addRevealClasses() {
        ...
        if (contactCard) contactCard.classList.add('reveal-left');
        if (contactInfoCol) contactInfoCol.classList.add('reveal-right');

        // Info cards
        document.querySelectorAll('.nova-info-card').forEach((card, i) => {
            card.classList.add('reveal');
            card.style.transitionDelay = `${i * 0.1}s`;
        });
    }
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            const text = `👋 Hola, me contacto desde la web de NovaBuild.

Soy ${nombre} ${apellido}.
📍 Ubicación del proyecto: ${zona}
📧 Email: ${email}
📝 Detalles: ${mensaje}

Solicito asesoramiento profesional para mi proyecto.`;
    ...
    if (contactCardEl) {
        contactCardEl.addEventListener('mousemove', e => {
            const rect = contactCardEl.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
            contactCardEl.style.setProperty('--card-x', `${x}px`);
            contactCardEl.style.setProperty('--card-y', `${y}px`);
        });
    }
